...
// void* clentHandler(void* args)
// {
//     client_args *c_args = (client_args*) args;
//     int clientSock = socket(AF_INET,SOCK_STREAM,0);
//     if(clientSock < 0)
//     {
//         printf("error create socket for client %s", c_args->name);
//         exit(EXIT_FAILURE);
//     }
...
//     if(bind(clientSock,(struct sockaddr*)&listenaddr, sizeof(listenaddr)) < 0)
//     {
//         printf("error create listen socket for %s", c_args->name);
//         exit(EXIT_FAILURE);
//     }

//     socklen_t clientAddrLen = sizeof(struct sockaddr_in);
//     if(connect(clientSock, (struct sockkaddr*)c_args->client,clientAddrLen) < 0)
//     {
//         printf("cant connect to %s", c_args->name);
//         exit(EXIT_FAILURE);
//     }
//     int recvBytes;
//     void* buf = malloc(sizeof(clientText));
//     clientText * clientRecv;
//     while((recvBytes = recv(clientSock,buf,sizeof(clientText),0)))
//     {
//         clientRecv = (clientText *) buf;
//         pthread_mutex_lock(c_args->mutex);

//         printf("%d-%02d-%02d %02d:%02d:%02d %s : %s",clientRecv->ttm->tm_year + 1900, clientRecv->ttm->tm_mon+1,clientRecv->ttm->tm_mday,clientRecv->ttm->tm_hour,clientRecv->ttm->tm_min,clientRecv->ttm->tm_sec,c_args->name,clientRecv->buffer);
//         fprintf(c_args->fLog,"%d-%02d-%02d %02d:%02d:%02d %s : %s",clientRecv->ttm->tm_year + 1900, clientRecv->ttm->tm_mon+1,clientRecv->ttm->tm_mday,clientRecv->ttm->tm_hour,clientRecv->ttm->tm_min,clientRecv->ttm->tm_sec,c_args->name,clientRecv->buffer);
//         pthread_mutex_unlock(c_args->mutex);
//     }
//     printf("client %s disconnected",c_args->name);
//     return NULL;
// }
...
int main(int argc, char** argv)
{
    ...
    // int maxclient = 10000;
    // int sockbroad = socket(AF_INET,SOCK_DGRAM,0);
    // if(sockbroad == -1)
    // {
    //     printf("cant create broadcast socket");
    //     exit(EXIT_FAILURE);
    // }
